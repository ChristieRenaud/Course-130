...
class TodoListTest < MiniTest::Test
  ...
  def test_add_raise_error
    assert_raises(TypeError) { @list.add("not an object") }
    assert_raises(TypeError) { @list.add(3) }
  end

  def test_shovel
    @todos4 = Todo.new("Walk dog")
    @todos << @todos4
    @list << @todos4
    assert_equal(@todos, @list.to_a )
  end
  ...
  def test_out_of_index
    assert_equal(true, @list.out_of_index?(-5))
    assert_equal(true, @list.out_of_index?(5))
    assert_equal(false, @list.out_of_index?(1))
  end
  ...
  def test_each
    ...
    assert_equal([@todo1, @todo2, @todo3], new_array)
  end
